...
export class DidSiopService {
  ...
  async checkAndInit(uuid?: string) {
    if (uuid) {
      if (this.siop_rp_mobile === undefined) {
        ...
        this.siop_rp_mobile = await DID_SIOP.RP.getRP(
          uuid ? environment.redirect_uri + '?session_id=' + uuid + '&client_side_app=true' : environment.redirect_uri, // RP's redirect_uri
        ...
        this.siop_rp_mobile.addSigningParams('CE438802C1F0B6F12BC6E686F372D7D495BC5AA634134B4A7EA4603CB25F0964'); // Private key
  ...
  async getRequest(uuid?: string): Promise<string> {
    ...
    return await this.checkAndInit(uuid).then(async () => {
      if (uuid) {
        return await this.siop_rp_mobile.generateRequest();
      } else {
  ...
  async processResponse(response: string, uuid?: string): Promise<any> {
    ...
    if (uuid) {
      return await this.siop_rp_mobile.validateResponse(response);
    } else {
